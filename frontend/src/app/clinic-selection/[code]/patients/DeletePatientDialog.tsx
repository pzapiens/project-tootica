...
/**
 * Delete Patient Profile confirm dialog (Figma "Delete Patient"). Removes the
 * patient (and their appointments) with `DELETE /api/patients/:id`.
 */
export default function DeletePatientDialog({
  ...
  async function remove() {
    ...
    try {
      ...
      setError(err instanceof ApiError ? err.message : "Couldn't delete the patient. Please try again.");
      setBusy(false);
    }
  }
    ...
    >
      ...
      >
        <div className="flex items-center gap-[10px]">
          ...
          <h2 id="delete-patient-title" className="font-manrope text-[22px] font-bold tracking-[-0.4px] text-[#1e1e24]">
            Delete Patient Profile?
          </h2>
        </div>
        <p className="font-inter text-[15px] leading-[23px] text-[#1e1e24]">
          Are you sure you want to delete the patient profile of{" "}
          <span className="font-semibold text-[#0077c0]">
            {patient.name} ({patient.code ?? "—"})
          </span>
          ? All appointments associated with this patient will also be permanently deleted. This
