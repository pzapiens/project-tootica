...
/**
 * Backdrop + centered card. Closes on Escape and backdrop click. Pass
 * `className` to tweak the card (e.g. `[zoom:0.95]` to shrink a tall form so it
...
/**
 * Mobile-number field with a fixed `+91` country code. `value` is the 10-digit
 * local part; input is filtered to digits (max 10). Validate with
 * `phoneDigitsError` and submit with `phoneWithCc`.
 */
export function LabeledPhone({
  id,
  label,
  value,
  onChange,
  ...
  return (
    <div className="flex flex-col gap-1.5">
      ...
      <div
        className={`flex items-center gap-2 rounded-lg border bg-white/60 px-4 py-4 shadow-[0_1px_2px_rgba(0,0,0,0.05)] ${
          error ? "border-red-500 focus-within:border-red-500" : "border-field-border focus-within:border-brand"
        }`}
      >
        <span className="shrink-0 font-inter text-[14px] text-ink">+91</span>
        <span className="h-5 w-px shrink-0 bg-field-border" />
        <input
          id={id}
          type="tel"
          inputMode="numeric"
